...
public class ClusterOrganizer extends ArrayList<Cluster> {
    ...
    private int querySortByInsert(int lowerBreakpoint, int upperBreakpoint, Cluster insert){
        if(lowerBreakpoint >= upperBreakpoint ){
            if(this.isEmpty()||this.get(upperBreakpoint).getQueryStart() < insert.getQueryStart()){
                super.add(upperBreakpoint+1,insert);
                return upperBreakpoint+1;
            }
            else {
                super.add(upperBreakpoint, insert);
                return upperBreakpoint;
            }
        }
        else{
            int middlePoint = (int) lowerBreakpoint+((upperBreakpoint-lowerBreakpoint+1)/2);
            //System.out.println(lowerBreakpoint + " " + middlePoint +" "+upperBreakpoint+"----"+this.filteredQ.size());
            if(this.get(middlePoint).getQueryStart() <= insert.getQueryStart()){
                return this.querySortByInsert(middlePoint, upperBreakpoint, insert);
            }
            else{
   ...
   /**
    ...
    private void targetSortByInsert(int lowerBreakpoint, int upperBreakpoint, Cluster insert, int qPosition){
        if(lowerBreakpoint >= upperBreakpoint ){
            if(this.isEmpty()||this.get(this.targetStarts.get(upperBreakpoint)).getTargetStart() < insert.getTargetStart()){
                this.targetStarts.add(upperBreakpoint+1,qPosition);

            }
            else {
                this.targetStarts.add(upperBreakpoint, qPosition);
            ...
            if(this.get(this.targetStarts.get(middlePoint)).getTargetStart() <= insert.getTargetStart()){
                 this.targetSortByInsert(middlePoint, upperBreakpoint, insert, qPosition);
            }
            else{
...
//                        "\n on Cluster "+c1.getQueryName() +" and "+c2.getQueryName());
    ...
    public double getRepeatlessScore(Cluster c1, Cluster c2){
        return (c2.size() - this.getMaximalOverlap(c1, c2));
    }
