...
public class ContigSorter implements Runnable {
	...
	/**
	 * Gives the {@link MatchList} which should be sorted.
	 * 
	 * @param matches
	 */
	public ContigSorter(MatchList matches) {
	 ...
	 */
	public void sort() {
		int arraySize = (int) matches.getStatistics().getTargetsSize()+1;
		histogram = new int[arraySize];
		DNASequence query;
		for (int i = 0; i < matches.getQueries().size(); i++) {
			query = matches.getQueries().get(i);
			...
			int bestIndex = getIndexOfHighestWindowCountFromHistogram((int) query
					.getSize());
			matches.getQueries().get(i).setSortKey(bestIndex);


			this.resetHistogram();
			reportProgress(((double) i / matches.getQueries().size()));
	 ...
	 *            hit, match
	 */
	private void enterApToHistogram(Match ap) {
...
//		 }
	...
	public Vector<DNASequence> getQueryOrder() {
		Vector<DNASequence> order = matches.getQueries();
