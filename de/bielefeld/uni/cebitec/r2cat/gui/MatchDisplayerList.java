 ...
 * 
 */
public class MatchDisplayerList implements
		Iterable<MatchDisplayer> {
	 ...
	 */
	public Vector<MatchDisplayer> getNearHits(Point2D.Double point,
			Double distance) {
		...
		for (Match ap : alignmentsPositions) {
			...
			for (int i = (int) firstBucket; i <= lastBucket; i++) {
				try {
					...
					//FIXME histogram index is sometimes 1 out of bounds 
	...
	public void markArea(Point2D.Double topLeft, Point2D.Double bottomRight,
		...
		if (alignmentsPositions.hasChanged()) {
			...
			alignmentsPositions
					.notifyObservers(MatchList.NotifyEvent.MARK);
		}
	}
